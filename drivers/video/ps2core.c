...
static void kdma_send_start(struct dma_request *req, struct dma_channel *ch)
{
	...
	WRITEDMAREG(ch, PS2_Dn_MADR, kreq->madr);
	WRITEDMAREG(ch, PS2_Dn_QWC,  kreq->size >> 4);
...
static void kdma_free(struct dma_request *req, struct dma_channel *ch)
{
    ...
    dma_unmap_single(NULL, kreq->madr, kreq->size, DMA_TO_DEVICE);
...
static void ps2kdma_send(struct kdma_buffer *kdb, size_t size)
{
	unsigned long flags;
	...
	spin_lock_irqsave(&kdb->lock, flags);
	alen = sizeof(struct kdma_request) + DMA_ALIGN(size);
	...
	kreq->kdb = kdb;
	kreq->madr = dma_map_single(NULL, (void *) kreq + sizeof(struct kdma_request), size, DMA_TO_DEVICE);
	kreq->size = size;
