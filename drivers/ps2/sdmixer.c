...
static long ps2sdmixer_ioctl(struct file *, unsigned int, unsigned long);
...
struct file_operations ps2sd_mixer_fops = {
	...
	unlocked_ioctl:	ps2sdmixer_ioctl,
...
static long
ps2sdmixer_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct ps2sd_mixer_context *mixer;
